...
const AfdaSelectedTableGroup: React.FC = () => {
  ...
  // Toolbar buttons scale with the room they actually have. A CSS breakpoint
  // can only see the viewport, so it would miss the chat side panel eating
  // ~600px of it — hence a JS threshold driven by both.
  ...
  // Two tiers: full labels when there's room, icon-only when there isn't.
  const isToolbarCompact = windowWidth < 1200;
  const toolbarLabelClass = isToolbarCompact ? 'hidden' : 'inline';
  const toolbarButtonClass = isToolbarCompact
    ? 'h-5 px-1.5 text-[11px]'
    : 'h-6 px-2 text-[12px]';
  const toolbarIconSize = isToolbarCompact ? 11 : 13;
  // Breadcrumb sheds its leading crumbs before the site name, which is the one
  // part that identifies the page.
  const breadcrumbCrumbClass = isToolbarCompact ? 'hidden' : 'inline';
  const breadcrumbNameClass = isToolbarCompact
  ...
  return (
    <div className="h-full flex overflow-hidden">
      <div className="flex flex-col overflow-hidden bg-white dark:bg-darkModeTable rounded-md gap-3 py-2 px-4 flex-1 min-w-0">
        {/* Breadcrumb */}
        <div className="flex items-center gap-1 text-xs text-gray-400 dark:text-gray-500 flex-shrink-0 min-w-0">
          <button
            onClick={() => navigate(-1)}
            className="flex items-center gap-1 hover:text-gray-700 dark:hover:text-gray-300 transition-colors mr-2 px-2 py-1 rounded text-gray-700 dark:text-gray-300 text-[11px] font-extrabold"
          >
            <span>
              <FaArrowLeft />
            </span>
            <span className={breadcrumbCrumbClass}>Back</span>
          </button>
          <button
            onClick={() => navigate('/status/all')}
            className={cn(
              'hover:text-gray-700 dark:hover:text-gray-300 transition-colors font-medium',
              breadcrumbCrumbClass,
            )}
          >
            Downloads
          </button>
          <span className={breadcrumbCrumbClass}>/</span>
          <button
            onClick={() =>
              navigate('/status/all', {
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            <TooltipWrapper content="Remove selected articles" side="bottom">
              <Button
                ...
                size="icon"
                className={cn(
                  'text-white rounded-md flex items-center justify-center py-[2px] bg-[#FF4F45] hover:bg-red-200 dark:hover:bg-red-900/40',
                  toolbarButtonClass,
                )}
                innerChildClass={toolbarLabelClass}
            ...
            >
              <Button
                ...
                size="icon"
                className={cn(
                  'text-white rounded-md flex items-center justify-center py-[2px] bg-primary hover:bg-primary/80',
                  toolbarButtonClass,
                )}
                innerChildClass={toolbarLabelClass}
            ...
            <div className="mr-3 flex-1  w-[clamp(180px,30vw,450px)]">
              <TaskbarInputField />
            </div>
          </div>
        </div>
