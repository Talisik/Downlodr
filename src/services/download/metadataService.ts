...
import { FFmpegWhisperTranscriber } from '@/Utils/Transcription/FFmpegWhisperTranscriber';
...
export class MetadataService {
  ...
  private static async runTranscription(
    ...
    try {
      ...
      // Update store immediately with transcript location
      if (downloadId) {
        try {
          useDownloadStore.setState((state) => ({
            downloading: state.downloading.map((d) => {
              ...
              return {
                ...d,
                autoCaptionLocation: transcriptionOutputPath,
              };
            }),
          }));
        } catch (error) {
          console.error(
            'Error updating download store with transcription path:',
            error,
          );
        }
      }

      // Update store that transcription is starting
      if (downloadId) {
        ...
        progressCleanup = window.downlodrFunctions.onFFmpegProgress(
          (progress: string) => {
            try {
              ...
              if (parsed.type === 'progress' && parsed.percent && downloadId) {
                const percent = parseFloat(parsed.percent);
                // Only update if progress increased (avoid spam)
      ...
      useDownloadStore.setState((state) => {
        ...
        return {
          ...
          downloading: state.downloading.map((d) => {
            ...
            // Update caption location if provided
            if (captionPath) {
              console.log(
          ...
          finishedDownloads: state.finishedDownloads.map((d) => {
            ...
            // Update transcript location if provided
            if (captionPath) {
              console.log(
