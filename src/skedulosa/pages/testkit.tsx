...
export default function ToolkitTestPage() {
  ...
  async function handleFirstScrapeTest() {
    ...
    try {
      ...
      const sched = await bridge.createSchedule({ name: '__first-scrape-debug__' }) as { id: number };
      scheduleId = sched.id;
      ...
      fsLog(`Step 2: Creating channel "${fsName}" → ${fsUrl.trim()} (first_scrape_limit=${fsLimit || '5'})…`);
      const ch = await bridge.createChannel({
      ...
      fsLog(`Channel created: id=${ch.id} — last_scraped_at is null (guaranteed first scrape)`);

      // Step 3: attach status listener
      bridge.removeScraperStatusListener();
      bridge.onScraperStatus((status) => {
        fsLog(`STATUS → phase="${status.phase}"${status.nextRunAt ? ` nextRunAt=${status.nextRunAt}` : ''}`);
      });
      ...
      const tasks = await bridge.listDownloadTasks() as Array<{ id: number; video_url: string; status: string; channel_id: number }>;
      const channelTasks = tasks.filter((t) => t.channel_id === ch.id);
      if (channelTasks.length === 0) {
        fsLog('⚠ No tasks were added for this channel — yt-dlp returned 0 new videos or all were filtered.');
      } else {
  ...
  async function handleFirstScrapeCleanup() {
    ...
    try {
      ...
      fsLog(`Cleanup error: ${err instanceof Error ? err.message : String(err)}`);
    }
  }
  ...
  return (
    <div className="p-5 max-w-3xl mx-auto">
      ...
      {/* ── 2. schedules ── */}
      <Section
        title="2. Schedules CRUD"
        description="createSchedule → listSchedules → deleteSchedule"
      >
        <div className="flex flex-wrap gap-2">
          <RunBtn onClick={handleCreateSchedule} label="Create schedule" />
          <RunBtn
            onClick={() => schedulesList.run(() => bridge.listSchedules())}
            label="List all"
            variant="neutral"
          />
          <RunBtn
            onClick={handleDeleteSchedule}
            label={`Delete #${createdScheduleId ?? '—'}`}
            variant="danger"
            disabled={createdScheduleId == null}
          />
        </div>
        {createdScheduleId != null && (
          <p className="text-xs text-emerald-600 dark:text-emerald-400">
            Created schedule id: <strong>{createdScheduleId}</strong> — used by
            section 3.
          </p>
        )}
        <ResultBlock result={scheduleCreate.result} />
        <ResultBlock result={schedulesList.result} />
        <ResultBlock result={scheduleDelete.result} />
        <div className="border-t border-red-200 dark:border-red-800 pt-3 mt-1">
          <p className="text-xs text-red-500 dark:text-red-400 mb-2 font-medium">
            ⚠ Danger — stops worker then wipes every schedule (and its
            channels/slots)
          </p>
          <RunBtn
            ...
            variant="danger"
          />
          <ResultBlock result={deleteAllSchedules.result} />
        </div>
      </Section>

      {/* ── 3. channels ── */}
      <Section
        title="3. Channels CRUD"
        description="createChannel (requires schedule from §2) → listChannels → setChannelActive → deleteChannel"
      >
        <div className="flex flex-wrap gap-2">
          <RunBtn
          ...
          <RunBtn
            onClick={() =>
              channelsList.run(() =>
                bridge.listChannels(undefined, createdScheduleId ?? undefined),
              )
            }
            label="List channels"
            variant="neutral"
            ...
            onClick={() =>
              channelsList.run(() =>
                createdChannelId != null
                  ? bridge
                      .setChannelActive(createdChannelId, false)
                      .then(() => bridge.getChannel(createdChannelId))
                  : Promise.reject(new Error('Create a channel first')),
              )
            }
            label="Deactivate"
            variant="neutral"
          ...
          <RunBtn
            ...
            label={`Delete ch #${createdChannelId ?? '—'}`}
            variant="danger"
            disabled={createdChannelId == null}
          />
        </div>
        {createdScheduleId == null && (
          <p className="text-xs text-yellow-600 dark:text-yellow-400">
            Create a schedule in §2 first to enable channel creation.
          </p>
        )}
        {createdChannelId != null && (
          <p className="text-xs text-emerald-600 dark:text-emerald-400">
            Created channel id: <strong>{createdChannelId}</strong> — used by
            section 4.
          </p>
        )}
        <ResultBlock result={channelCreate.result} />
        <ResultBlock result={channelsList.result} />
        <ResultBlock result={channelDelete.result} />
        <div className="border-t border-red-200 dark:border-red-800 pt-3 mt-1">
          <p className="text-xs text-red-500 dark:text-red-400 mb-2 font-medium">
            ⚠ Danger — stops worker then wipes every channel across all
            schedules
          </p>
          <RunBtn
            ...
            variant="danger"
          />
          <ResultBlock result={deleteAllChannels.result} />
        </div>
      </Section>

      {/* ── 4. channel slots ── */}
      <Section
        title="4. Channel Slots"
        description="replaceChannelSlots (snake_case) → listChannelSlots → getNextRun"
      >
        <div className="flex flex-wrap gap-2">
          <RunBtn
            onClick={() =>
              slotsReplace.run(() => {
                if (createdChannelId == null)
                  throw new Error('Create a channel first (§3)');
                return bridge.replaceChannelSlots(createdChannelId, [
                  { day_of_week: 1, time_minutes: 480 }, // Monday 8:00
                  { day_of_week: 4, time_minutes: 720 }, // Thursday 12:00
                ]);
              })
            }
            label="Set Mon 8:00 + Thu 12:00"
            disabled={createdChannelId == null}
          />
          <RunBtn
            onClick={() =>
              slotsList.run(() => {
                if (createdChannelId == null)
                  throw new Error('Create a channel first (§3)');
                return bridge.listChannelSlots(createdChannelId);
              })
            }
            label="List slots"
            variant="neutral"
          ...
          />
          <RunBtn
            onClick={() => nextRun.run(() => bridge.getNextRun())}
            label="Get next run"
            variant="neutral"
          />
        </div>
        {createdChannelId == null && (
          <p className="text-xs text-yellow-600 dark:text-yellow-400">
            Create a channel in §3 first.
          </p>
        )}
        <ResultBlock result={slotsReplace.result} />
        <ResultBlock result={slotsList.result} />
        <ResultBlock result={nextRun.result} />
      </Section>
      ...
      {/* ── 7. intelligent schedules ── */}
      <Section
        title="7. Intelligent Schedules"
        description="getUpcomingSchedules / getOverdueSchedules / getScheduleStats / refreshAllIntelligentSchedules"
      >
        <div className="flex flex-wrap gap-2">
          <RunBtn
            onClick={() =>
              intelligentUpcoming.run(() => bridge.getUpcomingSchedules(72))
            }
            label="Upcoming (72h)"
          />
          <RunBtn
            onClick={() =>
              intelligentOverdue.run(() => bridge.getOverdueSchedules())
            }
            label="Overdue"
            variant="neutral"
          />
          <RunBtn
            onClick={() =>
              intelligentStats.run(() => bridge.getScheduleStats())
            }
            label="Stats"
            variant="neutral"
          />
          <RunBtn
            onClick={() =>
              intelligentRefresh.run(() =>
                bridge.refreshAllIntelligentSchedules(),
              )
            }
            label="Refresh all"
            variant="neutral"
          />
          <RunBtn
            onClick={() => thisWeekSched.run(() => bridge.getThisWeekSched())}
            label="This week's schedule"
            variant="neutral"
          />
        </div>
        <ResultBlock result={intelligentUpcoming.result} />
        <ResultBlock result={intelligentOverdue.result} />
        <ResultBlock result={intelligentStats.result} />
        <ResultBlock result={intelligentRefresh.result} />
        <ResultBlock result={thisWeekSched.result} />
      </Section>
      ...
      {/* ── 11. fetch upload dates ── */}
      <Section
        title="11. Fetch Upload Dates"
        description="fetchUploadDates(url, daysBack?) — returns UTC upload timestamps for the channel going back N days."
      >
        <div className="flex flex-wrap gap-2 items-center">
          <input
            type="text"
            value={uploadDatesUrl}
            onChange={(e) => setUploadDatesUrl(e.target.value)}
            placeholder="https://www.youtube.com/@ChannelName"
            className="flex-1 min-w-0 rounded border border-gray-300 dark:border-darkModeCompliment bg-white dark:bg-darkMode text-xs text-gray-800 dark:text-gray-200 px-2 py-1.5 focus:outline-none focus:ring-1 focus:ring-primary"
          />
          <input
            type="number"
            value={uploadDatesDaysBack}
            onChange={(e) => setUploadDatesDaysBack(e.target.value)}
          ...
          <RunBtn
            onClick={() =>
              fetchUploadDatesResult.run(() =>
                bridge.fetchUploadDates(
                  uploadDatesUrl.trim(),
                  uploadDatesDaysBack ? Number(uploadDatesDaysBack) : undefined,
                ),
              )
            }
            label="Fetch dates"
            disabled={!uploadDatesUrl.trim()}
          />
        </div>
        <ResultBlock result={fetchUploadDatesResult.result} />
      </Section>
      ...
      >
        <div className="space-y-2">
          ...
          <div className="flex flex-wrap gap-2 items-center">
            ...
            />
            <span className="text-xs text-gray-400">videos on first scrape</span>
          </div>
          <div className="flex flex-wrap gap-2 items-center">
            <RunBtn
            ...
            <RunBtn
              ...
              variant="danger"
              disabled={fsRunning || (fsTempScheduleId == null && fsTempChannelId == null)}
            />
            {(fsTempScheduleId != null || fsTempChannelId != null) && (
              <span className="text-xs text-yellow-600 dark:text-yellow-400">
                Temp resources: schedule={fsTempScheduleId ?? '—'} channel={fsTempChannelId ?? '—'}
              </span>
            )}
          </div>
        </div>
        ...
        <p className="text-xs text-gray-400 dark:text-gray-500 mt-2">
          If step 4 shows "0 tasks" after a clean first scrape, yt-dlp returned no videos for that URL — check the main process console for yt-dlp errors.
        </p>
      </Section>
