      ...
      onRehydrateStorage: () => {
        return (state, error) => {
          if (error) {
            ...
            window.skedulosaBridge?.onChannelCreated?.(applyMcpChannelCreated);
            ...
            window.skedulosaBridge?.onChannelUpdated?.(applyMcpChannelUpdated);

            // Progress of a bridge-run subscribe, so the Status page shows the
            // same scraping row a manual subscribe gets.
            window.skedulosaBridge?.onChannelScraping?.(
              applyMcpChannelScraping,
            );
...
// ─── MCP bridge (Claude chat) channel events ─────────────────────────────────
// Extracted from the onRehydrateStorage listener registrations above so the
// behaviour is unit-testable. A subscription created here must be
// indistinguishable from one created by the Subscribe modal
// (SubscriptionQueueContext.createSubscription) — including the capitalised
// 'Active' status that the pause/resume controls compare against exactly.

interface McpCreatedChannel {
  id: number;
  url: string;
  name: string;
  schedule_id: number;
...
/**
 * Add (or progressively enrich) a subscription the MCP bridge just created.
 *
 * The bridge pushes the same channel up to three times: immediately, then again
 * once the avatar/details arrive, then once the intelligent schedule resolves.
 * Later pushes only fill in fields the subscription does not have yet.
 */
export function applyMcpChannelCreated(channel: unknown): void {
  const ch = channel as McpCreatedChannel | null;
  if (!ch?.id) return;
  const store = useSkedulosaStore.getState();
    ...
    if (needsUpdate) {
      const updated = store.subscriptions.map((s) =>
        s.toolkit_channel_id === ch.id
          ? {
              ...s,
              ...(ch.channel_details && !s.channel_details
                ? { channel_details: ch.channel_details }
                : {}),
              ...(ch.channel_analysis && !s.channel_analysis
                ? { channel_analysis: ch.channel_analysis }
                : {}),
              ...(hasSettings && s.settings.length === 0
    ...
    activity_log: [
      {
        id: `${Date.now()}-subscribe`,
        type: 'subscribe',
        action: 'Subscription created via Claude',
        detail: ch.url,
        timestamp: now,
      },
    ],
...
/**
 * Patch a subscription in-place after the MCP bridge updated the channel
 * (rename, active toggle, format, slot change) so the UI reflects it
 * immediately.
 */
export function applyMcpChannelUpdated(payload: unknown): void {
  const p = payload as {
    id: number;
    name?: string;
...
/**
 * Reflect the progress of a subscribe the MCP bridge is running in the main
 * process, so the Status page shows the same spinner row a manual subscribe
 * gets (StatusPageTable renders one row per `scrapingChannels` entry).
 *
 * The bridge sends a payload per phase and a final `{ done: true }`.
 */
export function applyMcpChannelScraping(payload: unknown): void {
  const p = payload as {
    channelId?: number;
    name?: string;
    status?: string;
  ...
  const store = useSkedulosaStore.getState();
  if (p.done) {
    store.removeChannelScraping(p.channelId);
    return;
  }
  if (store.scrapingChannels.has(p.channelId)) {
    if (p.status) store.updateChannelScrapingStatus(p.channelId, p.status);
    return;
  }
  store.setChannelScraping(p.channelId, p.name ?? '');
  if (p.status) store.updateChannelScrapingStatus(p.channelId, p.status);
}
