 ...
 * Displays videos in a selected category with list/folder view options
 * Supports video playback, thumbnails, and interactive features
 */

import { toast } from '@/core-app/components/shadcn/hooks/use-toast';
// import { FinishedDownloads as FinishedDownload } from '@/Store/download/types';
import {
...
// type FinishedDownload, VideoItem, VideoWithGroup are imported from SelectedCategoryTools
...
const SelectedCategory: React.FC<SelectedCategoryProps> = ({
  ...
  const [isAddDownloadModalOpen, setIsAddDownloadModalOpen] = useState(false);
  ...
  const filterOptions: FilterOptions = useMemo(() => {
    if (!groups[activeTab]) return { ext: [], source: [], category: [] };
    ...
    // Always include current active tab as category filter option
    categorySet.add(activeTab);
  ...
  }, [groups, activeTab, finishedDownloads]);
  ...
  }, [groups, activeTab, finishedDownloads, sortBy, selectedFilters]);
  ...
  */
  ...
  return (
    ...
    >
          ...
          <>
            {sortedVideos.length > 0 ? (
              <SelectedCategoryTools
                ...
                debugContext={`SelectedCategory-${activeTab}`}
              ...
              <div className="flex flex-1 justify-center mt-28 h-full">
                <div className="text-center ">
                  ...
                  <p className="text-[14px]">
                    Drag and drop from other categories or directly move videos
                    here
                  </p>
                  {/* <button
                    onClick={() => setIsAddDownloadModalOpen(true)}
                    className="mt-4 inline-flex items-center gap-2 px-4 py-2 bg-primary text-white text-sm rounded-md hover:opacity-90 transition-opacity"
                  >
                    <MdDownload size={16} />
                    Download a video
                  </button> */}
      ...
      {/* Side Player */}
      {isShowSidePlayer && (
        <div className="w-[33%] bg-[#F2F2F2] dark:bg-[#333333] p-2 h-full rounded-r">
      ...
      <AddDownloadModal
        ...
        onClose={() => setIsAddDownloadModalOpen(false)}
      />
    </div>
  );
};
