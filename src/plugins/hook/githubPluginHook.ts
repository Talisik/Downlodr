...
//
// The caption plugin ships under two names - the release id is the legacy
// 'cc-to-text-downlodr' while the plugin now presents itself as "CC to
// Markdown", and both can be installed side by side. Neither slug is
// derivable from the other, so both are listed against the same artwork;
// without the markdown entry that plugin fell through to its manifest's bare
// SVG and rendered as an unstyled glyph beside the bundled icons.
const PLUGIN_ICON_ASSETS: Record<string, string> = {
  'cc-to-text-downlodr': CCToMarkdownImg,
  'cc-to-markdown': CCToMarkdownImg,
...
// Returns a translated description for a known plugin id, or the original as fallback
function translatePluginDescription(id: string, original: string): string {
  const normalizedId = id.replace(/^-+|-+$/g, '');
  const key = `plugins:pluginDescriptions.${normalizedId}`;
  return i18n.exists(key) ? i18n.t(key) : original;
}

// Returns a translated name for a known plugin id, or the original as fallback
function translatePluginName(id: string, original: string): string {
  const normalizedId = id.replace(/^-+|-+$/g, '');
  const key = `plugins:pluginNames.${normalizedId}`;
  return i18n.exists(key) ? i18n.t(key) : original;
}
...
function getFallbackPluginsData() {
  return [
    ...
    },
    {
      ...
      name: translatePluginName(
        'format-converter-downlodr',
        'Format Converter',
      ),
      version: '1.0.1',
      description: translatePluginDescription(
    ...
    {
      id: 'fall-back-plugin',
      name: translatePluginName('fall-back-plugin', 'Fall Back Plugin'),
      version: '1.0.1',
      description: translatePluginDescription(
        'fall-back-plugin',
        'View and edit video metadata with powerful batch processing tools.',
...
function parsePluginDataFromRelease(releaseData: GitHubRelease): PluginData[] {
  ...
  if (pluginSections.length > 0) {
    ...
    pluginSections.forEach((section, index) => {
      const plugin: PluginData = {
        ...
        name: translatePluginName(
          section.id || `plugin-${index + 1}`,
          section.name || `Plugin ${index + 1}`,
        ),
  ...
  return plugins.length > 0 ? plugins : getFallbackPluginsData();
}
...
export async function getBrowsePluginsData(): Promise<FetchResult> {
  ...
  try {
    ...
    setCachedData(getFallbackPluginsData(), 'fallback');

    return {
      success: true,
      data: getFallbackPluginsData(),
      source: 'fallback',
      error:
