...
async function startServices() {
  ...
  scrapeEngine.setEventSink((channel, payload) =>
...
async function main() {
  ...
  try {
    ...
    process.stderr.write(
      `[downlodr-headless] ERROR: ${(err as Error).message}\n`,
    );
    process.exit(1);
  }
  ...
  // The bridge gates destructive and file-writing calls behind a user
  // confirmation and denies them outright when nothing can ask. Headless has no
  // UI and no user to ask, so it accepts them all — stated explicitly here
  // rather than left to a default, since it is a real loosening of the gate.
  approveAllGatedActions();
  process.stderr.write(
    '[downlodr-headless] Action gate: auto-approving (no UI to confirm in).\n',
  );

  startMcpBridgeServer();
  process.stderr.write(
    '[downlodr-headless] Bridge running on 127.0.0.1:7185. Press Ctrl+C to stop.\n',
  );
...
main().catch((err) => {
  process.stderr.write(
    `[downlodr-headless] Fatal: ${(err as Error).message}\n`,
  );
  process.exit(1);
});
