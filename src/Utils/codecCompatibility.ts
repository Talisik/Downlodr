...
export class CodecCompatibilityService {
  ...
  static isQuickTimeCompatible(format: VideoFormat): CodecCompatibilityInfo {
    ...
    const hasIncompatibleCodec = this.INCOMPATIBLE_CODECS.some(
      (incompatible) =>
    ...
    const hasCompatibleVideo =
      vcodec === 'none' ||
      this.QUICKTIME_VIDEO_CODECS.some((compatible) =>
        vcodec.includes(compatible),
      );

    const hasCompatibleAudio =
      acodec === 'none' ||
      this.QUICKTIME_AUDIO_CODECS.some((compatible) =>
        acodec.includes(compatible),
      );
    ...
    if (ext === 'mp4') {
      ...
      return {
        ...
        compatibilityReason: isFullyCompatible
          ...
          : `Incompatible ${
              !hasCompatibleVideo ? 'video' : 'audio'
            } codec in MP4`,
        suggestedCodecs: isFullyCompatible
          ? undefined
          : {
  ...
  static findBestQuickTimeFormat(formats: VideoFormat[]): VideoFormat | null {
    ...
    let bestFormat = formats.find((format) => {
      ...
      return (
        compatibility.isQuickTimeCompatible &&
        format.ext === 'mp4' &&
   ...
   */
  static filterQuickTimeCompatibleFormats(
    formats: VideoFormat[],
  ): VideoFormat[] {
  ...
  static getPreferredQuickTimeFormatCombination(
    ...
    const h264Formats = videoFormats
      .filter(
        (format) =>
  ...
  static validateFormatCombination(args: {
    ...
    if (args.remuxVideo !== 'mp4') {
      issues.push(
        `Container format '${args.remuxVideo}' may not be compatible with QuickTime`,
      );
    ...
    if (!compatibleAudioExts.includes(args.audioExt)) {
      issues.push(
        `Audio format '${args.audioExt}' may not be compatible with QuickTime`,
      );
   ...
   */
  static getOptimizedFormatArgs(availableFormats: VideoFormat[]): {
    formatId: string;
    ext: string;
    audioExt: string;
    audioFormatId: string;
    ...
    const optimized = this.getPreferredQuickTimeFormatCombination(
      videoFormats,
      audioFormats,
    );
...
export function isQuickTimeCompatible(codec: string): boolean {
  const compatibleCodecs = [
    'avc1',
    'h264',
    'libx264', // H.264 video codecs
    'mp4a',
    'aac',
    'libmp3lame', // Audio codecs
    'mpeg4',
    'h263', // Older video codecs
  ];
