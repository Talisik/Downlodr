...
interface WhisperUIHandlers {
  ...
  /** Called with 0–100 percent for progress bar UI */
...
export class FFmpegWhisperTranscriber {
  ...
  /**
   * Re-download / re-run transcription for an existing file with given options.
   * Uses the same flow as transcribe: validation, progress, and UI callbacks.
   */
  static async redownloadTranscript(
    ...
    try {
      const inputFileName = FFmpegWhisperTranscriber.getFileName(
        options.inputFile,
      );
      ...
      console.error('Error in redownloadTranscript:', error);
  ...
  static async transcribe(
    ...
    try {
      ...
      if (window.downlodrFunctions.onFFmpegProgress) {
        ...
        let lastProgressPercentUpdate = 0;
        let lastReportedPercent = -1;
        const PROGRESS_PERCENT_THROTTLE_MS = 5500;
        const PROGRESS_PERCENT_MIN_STEP = 5;
          ...
          (progress: string) => {
            ...
            try {
              ...
              if (parsed.type === 'progress') {
                ...
                // Throttle progress-percent callbacks to avoid lag (store updates → re-renders)
                const now = Date.now();
                const timeOk =
                  now - lastProgressPercentUpdate >=
                  PROGRESS_PERCENT_THROTTLE_MS;
                const stepOk =
                  percent - lastReportedPercent >= PROGRESS_PERCENT_MIN_STEP;
                const nearEnd = percent >= 99;
                if (timeOk || stepOk || nearEnd) {
                  ui?.onProgressPercent?.(percent);
                  lastProgressPercentUpdate = now;
