...
const ERROR_CODE_MAP: Record<string, ErrorCodeInfo> = {
  ...
  youtube_bot_detection: {
    ...
    title: 'YouTube Blocked Your Network',
    description: "YouTube doesn't recognize your network right now. Try a different connection",
    category: 'ytdlp',
    severity: 'high',
    canRetry: false,
    suggestions: [
      'Switch to mobile hotspot or a different WiFi',
      'Try a VPN',
  ...
  age_restricted_unverified: {
    ...
    description:
      "This video is age-restricted, and the signed-in Google account hasn't completed age verification with Google — being signed in alone isn't enough.",
    category: 'ytdlp',
    severity: 'medium',
    canRetry: true,
    suggestions: [
      'Verify your age at myaccount.google.com',
      'Try a different signed-in browser whose account is already age-verified',
    ],
  },
  extraction_blocked_reload: {
...
export function parseErrorCodeFromLog(
  ...
  // Age-restriction messages must be checked before the bot-detection check
  // below — "Sign in to confirm your age" also matches
  // `includes('Sign in to confirm')`, and would otherwise be wrongly
  // classified as an IP block, sending the user to switch networks or use a
  // VPN instead of signing in.
  //
  // "...age-restricted and only available on YouTube" is the more specific
  // of the two (it only shows up once a signed-in session already reached
  // YouTube's age check), so it's checked first.
  if (logMessage.includes('age-restricted and only available on YouTube')) {
    return 'age_restricted_unverified';
  }
  if (
    logMessage.includes('[youtube]') &&
    logMessage.includes('Sign in to confirm your age')
  ) {
  ...
  // Look for YouTube bot detection (IP block) — most specific, check first
  if (
    logMessage.includes('[youtube]') &&
    logMessage.includes('Sign in to confirm')
  ) {
