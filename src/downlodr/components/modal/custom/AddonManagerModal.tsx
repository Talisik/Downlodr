...
const AddonManagerModal: React.FC<AddonManagerModalProps> = ({
  ...
  const smartOrganizeState = useAddonStore((s) => s.smartOrganize);
  ...
  const isSmartOrganizeRequired =
    addonManagerReason === 'smart-organize-required';
  const title =
    isAfdaRequired || isSmartOrganizeRequired
      ? 'Download required add-on'
      : 'Add-ons';
  const description = isAfdaRequired
    ? 'To download articles and subscribe to website please download addon'
    : isSmartOrganizeRequired
    ? 'To use Smart Organize please download this add-on'
