...
/**
 * Highlight for the row whose context menu is open — the active background,
 * a shade darker, so it reads as "this is the one you right-clicked" even
 * when that row is also the active route.
 */
const CONTEXT_TARGET_CLASS = 'bg-[#ECEDE9] dark:bg-[#333333]';
...
const Navigation = ({
  ...
  // Category whose share dialog is open, with the source URLs it will share.
  ...
  /**
   * Gathers the source URLs in a category and opens the share dialog.
   *
   * Deliberately reads the same lists the category page renders, so a share
   * contains exactly what the user sees when they open the category:
   * `historyDownloads` is left out (the user removed those), and AFDA articles
   * are left out because they live in a separate store and are not videos.
   */
  const handleShareCategory = (categoryName: string) => {
    ...
    const videoUrls = [
      ...
      .filter((download) => download.category?.includes(categoryName))
      .map((download) => download.videoUrl)
      .filter(Boolean);

    if (videoUrls.length === 0) {
      toast({
        title: t('toast.shareEmpty'),
        description: t('toast.shareEmptyDesc', { category: categoryName }),
        variant: 'destructive',
      });
      return;
    }

    setShareCategory({ name: categoryName, videoUrls });
  };
    ...
    >
              ...
              <div className="flex flex-col gap-[6px]">
                ...
                {allAvailableCategories.map((category) => (
                  <NavLink
                    ...
                    className={({ isActive }) =>
                      // The context-menu target wins over the active state
                      // rather than stacking with it: two bg-* utilities on
                      // one element have equal specificity, so which one
      ...
      {shareCategory && (
        <ShareButton
          ...
          open
          onOpenChange={(isOpen) => !isOpen && setShareCategory(null)}
        />
      )}
