...
type ToastVariant = keyof typeof VARIANT_PRIORITY;
  ...
  switch (action.type) {
    case 'ADD_TOAST': {
      ...
      const sorted = merged.sort(
        (a, b) =>
          VARIANT_PRIORITY[(b.variant ?? 'default') as ToastVariant] -
          VARIANT_PRIORITY[(a.variant ?? 'default') as ToastVariant],
      );
      return {
        ...state,
        toasts: sorted.slice(0, TOAST_LIMIT),
      };
    }
