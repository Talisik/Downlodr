...
contextBridge.exposeInMainWorld('skedulosaBridge', {
  ...
  /** Fired by the MCP bridge while it runs a subscribe: per-phase progress, then { done: true } */
  onChannelScraping: (callback: (payload: unknown) => void) => {
    ipcRenderer.removeAllListeners('toolkit:channel:scraping');
    ipcRenderer.on('toolkit:channel:scraping', (_event, payload) =>
      callback(payload),
    );
  },

  removeChannelScrapingListener: () => {
    ipcRenderer.removeAllListeners('toolkit:channel:scraping');
  },
