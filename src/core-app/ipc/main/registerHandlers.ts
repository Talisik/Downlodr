...
import { smartOrganizeHandler } from '../../../smart-organize/ipc/main/smartOrganizeHandler';
import { autoTagHandler } from '../../../auto-tag/ipc/main/tagHandler';
import { registerSmartOrganizeReady } from './mcpBridgeServer';
...
export async function registerMainIpcHandlers(
  ...
  // skedulosaHandler/smartOrganizeHandler dynamically import multi-MB add-on
  ...
  const registerAddonHandlers = async () => {
    ...
    await sendBootStatus('Loading smart organizer…');
    const smartOrganizeState = detectAddon('smart-organize-backend');
    const addonPathSmartOrganize =
      smartOrganizeState.status === 'ready' ? resolveAddonPath('smart-organize-backend') : undefined;
    if (addonPathSmartOrganize || !app.isPackaged) {
      try {
        collect(smartOrganizeHandler(mainWindow, addonPathSmartOrganize));
        registerSmartOrganizeReady(true);
      } catch (err) {
        console.error(`[addons] smartOrganizeHandler failed (addonPath: ${addonPathSmartOrganize ?? 'undefined'}):`, err);
        registerSmartOrganizeReady(false);
      }
    } else {
      registerSmartOrganizeReady(false);
      console.log(`[addons] smart-organize-backend not ready (${smartOrganizeState.status}) — skipping handler registration`);
    }
