...
const MainLayout = () => {
  ...
  // The content row's box is the area the chat FAB can be thrown around in —
  // everything below the titlebar and taskbar, so it can never land on the
  // window controls.
  const contentRef = useRef<HTMLDivElement>(null);
  // gapPx: 8 matches the row's `gap-2` — the wrapper is always mounted, so the
  // hook cancels the phantom gap while closed. Same setup as the plugin panel.
