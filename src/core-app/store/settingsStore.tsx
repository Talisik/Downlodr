...
interface DownloadSettings {
  ...
  /** 'none' = no cookie auth; 'live' = --cookies-from-browser each call
   *  (Firefox/Brave); 'import' = a one-shot exported jar (Chrome/Edge). */
  cookieAuthMode: 'live' | 'import' | 'file' | 'none';
  /** e.g. 'firefox', 'chrome'; null when cookieAuthMode is 'none'. */
  cookieAuthBrowser: string | null;
}
...
export const MAIN_SETTINGS_VERSION = 8; // Incremented: replaced cookiesFromBrowser with cookieAuthMode/cookieAuthBrowser
...
// migration function
export const migrateMainStore = (persistedState: unknown, version: number) => {
  console.log(
  ...
  if (version === undefined || version === 0) {
    ...
    const defaultState = {
      settings: {
        ...
        cookieAuthMode: 'none' as const,
        cookieAuthBrowser: null as string | null,
      },
    };
  ...
  if (version === 5) {
    return {
      ...
      settings: {
        ...
        hasSeenFormatHint: false,
        cookieAuthMode: 'none',
        cookieAuthBrowser: null,
      },
    };
  }

  // Version 6 → 7: previously added cookiesFromBrowser for the advanced
  // settings modal; that field is now replaced by the v7 → 8 migration
  // below. A user persisted at exactly version 6 skips straight to the
  // current version without passing through the v7 branch, so this branch
  // must add cookieAuthMode/cookieAuthBrowser itself (same defaults as v5
  // and the version-0 default state) rather than being a no-op.
  if (version === 6) {
    return {
      ...(persistedState as any),
      settings: {
  ...
  // Version 7 → 8: cookiesFromBrowser (write-only, no consumer) replaced by
  // cookieAuthMode/cookieAuthBrowser, now actually wired to yt-dlp calls.
  // Every prior migration branch set cookiesFromBrowser: '' or left it
  // absent, so there is no meaningful prior value to carry forward —
  // default every v7-and-earlier user to 'none', same as today's inert
  // behavior.
  if (version === 7) {
    const prevSettings = { ...(persistedState as any).settings };
    delete prevSettings.cookiesFromBrowser;
    return {
      ...(persistedState as any),
      settings: {
        ...prevSettings,
...
export const useSettingStore = create<SettingsStore>()(
  persist(
    (set, get) => ({
      ...
      updateCookieAuth: (mode, browser) =>
        set((state) => ({
          settings: {
            ...state.settings,
            cookieAuthMode: mode,
            cookieAuthBrowser: browser,
          },
        })),
    }),
    {
