...
export type PackName =
  ...
  | 'smart-organize-backend';

/** Maps each PackName to the corresponding key on AddonStore's per-pack state. */
export const PACK_STORE_KEY: Record<PackName, 'afda' | 'skedulosa' | 'smartOrganize'> = {
  'afda-backend': 'afda',
  'video-nemesis-toolkit': 'skedulosa',
  'smart-organize-backend': 'smartOrganize',
};
...
export type AddonManagerReason =
  | 'afda-required'
  | 'smart-organize-required'
  | null;

interface AddonStore {
  afda: AddonPackState;
  skedulosa: AddonPackState;
  smartOrganize: AddonPackState;
...
export const useAddonStore = create<AddonStore>((set) => ({
  ...
  initFromMain: async () => {
    ...
    set({
      ...
      smartOrganize: toPackState(result.smartOrganize),
    });
