...
} from './smart-organize/schema/smartOrganizeTypes';
...
declare global {
  ...
  const __OTEL_LOGS_ENDPOINT__: string;
  const __SUMMARY_API_ENDPOINT__: string;
  ...
  interface Window {
    ...
    /** Cookie authentication: browser detection, live/import/file mode, one-shot import (cookieAuth/handler.ts) */
    cookieAuthBridge: {
      getState: () => Promise<{
        mode: 'live' | 'import' | 'file' | 'none';
        browser: string | null;
        detected: {
          name: string;
          exists: boolean;
      ...
      siteLogin: {
        ...
        list: () => Promise<
          { domain: string; jarPath: string; loginUrl: string; lastLoginAt: number }[]
        >;
        remove: (domain: string) => Promise<{ ok: boolean }>;
      };
    };
    ...
    addonBridge: {
      getStatus: () => Promise<{
        ...
        smartOrganize: { status: string; installedVersion?: string; path: string | null };
      }>;
      download: (pack: 'afda-backend' | 'video-nemesis-toolkit' | 'smart-organize-backend') => Promise<{ started: boolean }>;
      restart: () => Promise<void>;
      delete: (pack: 'afda-backend' | 'video-nemesis-toolkit' | 'smart-organize-backend') => Promise<{ success: boolean; deferred?: boolean; error?: string }>;
      openFolder: (pack: 'afda-backend' | 'video-nemesis-toolkit' | 'smart-organize-backend') => Promise<{ success: boolean }>;
      cancel: (pack: 'afda-backend' | 'video-nemesis-toolkit' | 'smart-organize-backend') => Promise<void>;
      on: {
        progress: (cb: (data: { pack: 'afda-backend' | 'video-nemesis-toolkit' | 'smart-organize-backend'; percent: number }) => void) => () => void;
        complete: (cb: (data: { pack: 'afda-backend' | 'video-nemesis-toolkit' | 'smart-organize-backend'; success: boolean; error?: string }) => void) => () => void;
    ...
    skedulosaBridge: {
      ...
      onChannelScraping: (callback: (payload: unknown) => void) => void;
      removeChannelScrapingListener: () => void;
    ...
    // ─── Chat / local LLM bridge (chat/chatBridgeApi.ts, both preloads) ────
    chatBridge: {
      send: (payload: { messages: unknown[]; model: string; baseUrl: string; apiKey?: string; agenticMode?: boolean; sessionId?: string }) => void;
      abort: () => void;
      onChunk: (cb: (chunk: string) => void) => () => void;
      onDone: (cb: () => void) => () => void;
      onError: (cb: (message: string) => void) => () => void;
      /** Main asking a re-shown chat window to re-restore the newest session from disk. */
      onRestoreLatest: (cb: () => void) => () => void;
      /** The agent restarted in a fresh CLI session instead of resuming this one. */
      onSessionNotice: (cb: (data: { recapped: boolean }) => void) => () => void;
      /** Answer a card whose wait already timed out; main runs it and resumes the chat. */
      confirmLateAnswer: (id: string, approved: boolean, patch?: Record<string, unknown>) => void;
      /** Main asking for a follow-up turn after it ran a late-approved action. */
      onResumeAfterApproval: (cb: (data: { text: string }) => void) => () => void;
      onToolCall: (cb: (data: { id?: string; name: string; args: unknown }) => void) => () => void;
      /** `ok` is main's verdict on the command; absent means treat as success. */
      onToolResult: (cb: (data: { id?: string; name: string; result: unknown; ok?: boolean }) => void) => () => void;
      /** A destructive or file-writing command is waiting on the user's yes/no. */
      onConfirmRequest: (
        cb: (data: {
          id: string;
          command: string;
          kind: 'destructive' | 'file-write' | 'addon-required';
          detail?: string;
          /** Set when the card should review chosen values, not just a command. */
          review?: {
            title: string;
            chip?: string;
            approveLabel?: string;
            fields: Array<{ label: string; value: string }>;
            /** A batch's removable video rows. Absent for a single-item action. */
            items?: Array<{ label: string; url: string }>;
            /** Batch-level choices the card collects and patches into the body. */
            toggles?: Array<{
      ...
      /** How a permission request ended — buttons, typed reply, or expiry. */
      onConfirmResolved: (
        cb: (data: { id: string; status: 'approved' | 'declined' | 'expired' }) => void,
      ) => () => void;
      /** Answer a permission request from the card's buttons. */
      confirmReply: (
        id: string,
        approved: boolean,
      ...
      minimize: () => void;
      close: () => void;
      /** Relaunch Downlodr (invokes the same addon:restart handler as addonBridge.restart()). */
      restart: () => Promise<unknown>;
      saveSettings: (settings: { model: string; baseUrl: string; apiKey?: string; agenticMode?: boolean }) => void;
      loadSettings: () => Promise<{ model: string; baseUrl: string; apiKey?: string; agenticMode?: boolean }>;
      listSessions: () => Promise<Array<{ id: string; title: string; createdAt: number; updatedAt: number; messageCount: number; model: string }>>;
      readSession: (id: string) => Promise<{ id: string; messages: unknown[]; claudeCliSessionId?: string; claudeCliHasRun?: boolean } | null>;
      saveSession: (payload: { id: string; messages: unknown[]; model: string }) => Promise<Array<{ id: string; title: string; createdAt: number; updatedAt: number; messageCount: number; model: string }>>;
      deleteSession: (id: string) => Promise<Array<{ id: string; title: string; createdAt: number; updatedAt: number; messageCount: number; model: string }>>;
      checkReadiness: () => Promise<
        | { ready: true }
        | { ready: false; kind: 'no_key' | 'invalid_key' | 'not_installed' | 'not_authenticated' | 'error'; message: string }
      >;
      checkClaudeCli: () => Promise<
        | { ready: true }
        | { ready: false; kind: 'not_installed' | 'not_authenticated' | 'error'; message: string }
      >;
      claudeLogout: () => Promise<{ ok: boolean; error?: string }>;
      claudeLoginStart: () => Promise<{
        ok: boolean;
        authUrl?: string;
        error?: string;
      }>;
      claudeInstallStart: () => Promise<{
        ok: boolean;
        method?: 'npm' | 'native';
        error?: string;
      }>;
      claudeLoginSubmitCode: (code: string) => void;
      claudeLoginCancel: () => void;
      onClaudeLoginResult: (cb: (result: { ok: boolean; message: string }) => void) => () => void;
      onClaudeLoginManualFallback: (
        cb: (data: { authUrl: string }) => void,
      ) => () => void;
    };
    ...
    /** Smart Organize — ML-powered video categorization (smartOrganizeBridge) */
    smartOrganizeBridge: {
    ...
    /** Share-link metadata recording (shareHandler.ts) — runs in main to avoid renderer-side CORS */
    shareBridge: {
      record: (payload: {
        senderUserId: string;
        longUrl: string;
