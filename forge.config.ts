...
async function signBinaryWithEntitlements(binaryPath: string, binaryName: string, entitlementsPath?: string): Promise<void> {
  try {
    ...
    const codesignArgs = [
      '--sign', process.env.APPLE_IDENTITY!,
      '--force',
      '--options', 'runtime',
      '--deep',
      '--strict'
    ...
    await new Promise((resolve, reject) => {
      ...
      signProcess.on('close', (code) => {
        if (code === 0) {
          ...
          console.error(`   ❌ Failed to sign ${binaryName} binary (exit code: ${code})`);
          if (stderr) console.error(`   📝 Stderr: ${stderr}`);
          if (stdout) console.error(`   📝 Stdout: ${stdout}`);
          reject(new Error(`Code signing failed with exit code ${code}: ${stderr}`));
        }
      });
    ...
    const verifyProcess = spawn('codesign', ['--verify', '--deep', '--strict', binaryPath]);
    await new Promise((resolve) => {
      verifyProcess.on('close', (code) => {
        if (code === 0) {
          console.log(`   ✅ ${binaryName} binary signature verified`);
        } else {
          console.warn(`   ⚠️  ${binaryName} binary signature verification failed (exit code: ${code})`);
        }
    ...
    console.warn(`   ⚠️  Failed to sign ${binaryName} binary (continuing anyway):`, signError.message);
...
const config: ForgeConfig = {
  packagerConfig: {
    ...
    osxSign: process.env.APPLE_IDENTITY && !process.env.SKIP_CODE_SIGNING
      ? ({
      ...
      for (const outputPath of packageResult.outputPaths) {
        try {
          ...
          if (process.platform !== 'darwin') {
            console.log(`📦 Skipping code signing for platform: ${process.platform}`);
            continue;
          }
          ...
          if (!sourceBinaryPath) {
            console.warn(`⚠️  No yt-dlp binary found in project root. Checked paths:`, sourcePaths);
            continue;
          }
          ...
          for (const destPath of destinationPaths) {
            try {
              ...
              if (process.platform === 'darwin' && process.env.APPLE_IDENTITY && !process.env.SKIP_CODE_SIGNING) {
                await signBinaryWithEntitlements(destPath, 'yt-dlp', path.join(__dirname, 'yt-dlp-entitlements.plist'));
              }
              
            } catch (copyError) {
              console.warn(`   ⚠️  Failed to copy to ${destPath}:`, copyError.message);
            }
          }
          ...
          try {
            ...
            console.warn(`   ⚠️  Failed to create fallback binary:`, fallbackError.message);
          }
        ...
        if (process.platform === 'darwin' && process.env.APPLE_IDENTITY && !process.env.SKIP_CODE_SIGNING) {
          ...
          for (const candidate of ffmpegCandidates) {
            if (await fs.access(candidate.path).then(() => true).catch(() => false)) {
              try {
                console.log(`   📦 Processing ${candidate.name} at ${candidate.path}`);
                ...
                console.warn(`   ⚠️  Failed to process ${candidate.name}: ${ffmpegError.message}`);
              }
            } else {
              console.log(`   ❌ ${candidate.name} not found at ${candidate.path}`);
            }
          }
        }
      }
